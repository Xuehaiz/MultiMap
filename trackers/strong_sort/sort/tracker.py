...
class Tracker:
    ...
    def update(self, detections, classes, confidences):
        ...
        self.reference = {k: v for k, v in self.reference.items() if v.age <= 150}

        for track in active_tracks:
            if track.track_id in self.reference:
                continue
            else:
                self.record[int(track.class_id)] += 1
                self.reference[track.track_id] = Count(track.track_id, track.class_id, self.record[int(track.class_id)], 1)
                self.counts.append(self.reference[track.track_id])
